...
public class BleManager implements BleGattExecutor.BleExecutorListener {
    ...
    // TODO: find out of refereshDeviceCache is used in this code. if so, write it
    ...
    public void disconnect() {
        ...
        if(mAdapter == null || mGatt == null) {
            Log.w(TAG, "disconnect: Bluetooth adapter not initialized");
            return;
        }
    ...
    /**
     * After using a given BLE device, the app must cal this method to ensure resources are
     * released properly
     */
    public void close() {
        if(mGatt != null) {
    ...
    public boolean readRssi() {
        if(mGatt != null) {
            ...
            return false; // Rssi read is not available
        }
    }
    ...
    public void readService(BluetoothGattService service, String characteristicUUID, String descriptorUUID) {
        if(service != null) {
            if(mAdapter == null || mGatt == null) {
                Log.w(TAG, "readService: Bluetooth Adapter not initialized");
                return;
            }
